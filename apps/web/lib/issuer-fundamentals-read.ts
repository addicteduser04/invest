...
interface IssuerFundamentalsRow {
  id: string;
  issuer_id: string;
...
const issuerFundamentalsViewSchema = z.object({
  issuerId: z.string(),
...
export type IssuerFundamentalsView = z.infer<typeof issuerFundamentalsViewSchema>;

const toFigures = (row: IssuerFundamentalsRow): FundamentalsFigures => ({
...
function findPriorMatchingPeriod(rows: IssuerFundamentalsRow[], latest: IssuerFundamentalsRow) {
  return (
    rows.find(
      (row) =>
        row.id !== latest.id &&
...
/**
 * Issuer-scoped counterpart of apps/web/lib/fundamentals-read.ts's readSecurityFundamentals --
 * same shape/derivation, reading market.fundamentals via the issuer_id-keyed
 * public.issuer_fundamentals view instead of the security_id-keyed one. Used by the /companies
 * issuer page, including issuers with no listed BVC security.
 */
export async function readIssuerFundamentals(issuerId: string): Promise<IssuerFundamentalsView> {
  const supabase = await createClient();
  const { data, error } = await supabase
    .from('issuer_fundamentals')
    .select(
      'id,issuer_id,period_type,interim_period,fiscal_year,period_end_date,publication_date,currency,revenue,ebitda,ebit,net_income,eps,cash_and_equivalents,total_debt,total_assets,total_equity,operating_cash_flow,capex,shares_outstanding,dividend_per_share',
    )
  ...
  const rows = (data ?? []) as IssuerFundamentalsRow[];
  ...
  return issuerFundamentalsViewSchema.parse({
    issuerId,
    latest: latestRow
      ? {
          id: latestRow.id,
