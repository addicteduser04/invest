...
export async function POST(request: Request) {
  ...
  try {
    ...
    return Response.json({
      ...preview,
      rowCount: preview.candidates.length,
      filename: `bvc-public-test-index-${String(input.code ?? 'MASI').toUpperCase()}-${String(input.period ?? '1m')}.csv`,
      notice: 'Read-only index history testing export. Nothing was persisted or published.',
    });
  } catch (error) {
